...
use super::settings::{Settings, NutrientPattern};
...
impl GrayScottModel {
    ...
    pub fn render(&mut self, view: &TextureView) -> Result<(), wgpu::SurfaceError> {
        ...
        self.renderer.render(view, output_buffer, &self.params_buffer)
    }
    ...
    pub fn set_nutrient_pattern(&mut self, pattern: NutrientPattern, is_reversed: bool) {
        ...
        self.renderer.queue().write_buffer(&self.params_buffer, 0, bytemuck::cast_slice(&[params]));
    }
    ...
    pub(crate) fn render_frame(&mut self, device: &Device, queue: &Queue, surface_view: &TextureView) -> Result<(), wgpu::SurfaceError> {
        ...
        // Render the current state - pass the output buffer (which contains the latest results)
        let output_buffer = &self.uvs_buffers[self.current_buffer];
        self.renderer.render(surface_view, output_buffer, &self.params_buffer)
    }
    ...
    pub fn pan_camera(&mut self, delta_x: f32, delta_y: f32) {
        tracing::debug!("Gray-Scott simulation pan_camera called: delta=({:.2}, {:.2})", delta_x, delta_y);
        self.renderer.camera.pan(delta_x, delta_y);
    }

    pub fn zoom_camera(&mut self, delta: f32) {
        tracing::debug!("Gray-Scott simulation zoom_camera called: delta={:.2}", delta);
    ...
    pub fn zoom_camera_to_cursor(&mut self, delta: f32, cursor_x: f32, cursor_y: f32) {
        ...
        self.renderer.camera.zoom_to_cursor(delta, cursor_x, cursor_y);
    }
