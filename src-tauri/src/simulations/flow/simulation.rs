use super::settings::{Settings, NoiseType};
use super::shaders::{PARTICLE_UPDATE_SHADER, PARTICLE_RENDER_SHADER, TRAIL_DECAY_DIFFUSION_SHADER, TRAIL_RENDER_SHADER, BACKGROUND_RENDER_SHADER};
use crate::simulations::traits::Simulation;
use crate::simulations::shared::camera::Camera;
use crate::simulations::shared::LutManager;
use bytemuck::{Pod, Zeroable};
use noise::{NoiseFn, Perlin, Simplex, OpenSimplex, Worley, Value as ValueNoise};
use rand::Rng;
...
impl FlowModel {
    ...
    fn generate_flow_direction(pos: [f32; 2], noise_type: NoiseType, noise_scale: f64, noise_seed: u32) -> [f32; 2] {
    ...
    fn regenerate_flow_vectors(&mut self, queue: &Arc<Queue>) -> crate::error::SimulationResult<()> {
        ...
        for y in 0..grid_size {
            for x in 0..grid_size {
                ...
                let direction = [direction[0] * self.settings.vector_magnitude, direction[1] * self.settings.vector_magnitude];
        ...
        queue.write_buffer(&self.flow_vector_buffer, 0, bytemuck::cast_slice(&flow_vectors));
    ...
    pub fn new(
        ...
        let mut particles = Vec::with_capacity(settings.particle_count as usize);
        
        // Get LUT data for random colors
        let lut_data = lut_manager.get(&settings.current_lut).unwrap_or_else(|_| lut_manager.get_default());
        ...
        for y in 0..grid_size {
            for x in 0..grid_size {
                ...
                let direction = [direction[0] * settings.vector_magnitude, direction[1] * settings.vector_magnitude];
        ...
        let particle_update_pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
        ...
        let trail_decay_diffusion_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Flow Trail Decay Diffusion Shader"),
            source: wgpu::ShaderSource::Wgsl(TRAIL_DECAY_DIFFUSION_SHADER.into()),
        });
        
        let trail_update_bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        ...
        let trail_render_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            ...
            layout: Some(&device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                ...
                bind_group_layouts: &[&trail_render_bind_group_layout, &camera_bind_group_layout],
                push_constant_ranges: &[],
            })),
            vertex: wgpu::VertexState {
...
impl Simulation for FlowModel {
    fn render_frame(
        ...
        
        {
            ...
            compute_pass.dispatch_workgroups(
                (self.settings.particle_count + 63) / 64,
                1,
                1,
            );
        }
...

        {
            ...
            // Render particles with 3x3 grid (9 instances per particle) on top
            render_pass.set_pipeline(&self.particle_render_pipeline);
            render_pass.set_bind_group(0, &self.particle_render_bind_group, &[]);
            render_pass.set_bind_group(1, &self.camera_bind_group, &[]);
            render_pass.draw(0..6, 0..self.settings.particle_count * 9); // 3x3 grid = 9 instances
        }
    ...
    fn render_frame_static(
        ...
        // For static rendering, render particles in 3x3 grid without updating
        let mut render_encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
    ...
    fn resize(
        ...
        self.trail_texture_view = self.trail_texture.create_view(&wgpu::TextureViewDescriptor {
            label: Some("Trail Texture View"),
            dimension: Some(wgpu::TextureViewDimension::D2),
            format: Some(wgpu::TextureFormat::Rgba8Unorm),
            ..Default::default()
        ...
        self.trail_decay_diffusion_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("Trail Decay Diffusion Bind Group"),
            layout: &device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                label: Some("Trail Update Bind Group Layout"),
                entries: &[
        ...
        match setting_name {
            ...
            "particleCount" => {
                if let Some(count) = value.as_u64() {
                    ...
                    if new_count != self.settings.particle_count {
                        self.settings.particle_count = new_count;
                        
                        // Recreate particles with new count
                        let mut particles = Vec::with_capacity(new_count as usize);
                        
                        // Get LUT data for random colors
                        let lut_data = self.lut_manager.get(&self.settings.current_lut).unwrap_or_else(|_| self.lut_manager.get_default());
            ...
            "currentLut" => {
                if let Some(lut_name) = value.as_str() {
                    ...
                    let mut lut_data = self.lut_manager.get(&self.settings.current_lut).unwrap_or_else(|_| self.lut_manager.get_default());
                    ...
                    self.particle_update_pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
                        label: Some("Flow Particle Update Pipeline"),
            ...
            "particleShape" => {
                if let Some(shape_str) = value.as_str() {
                    self.settings.particle_shape = match shape_str {
                        ...
                        "Star" => super::settings::ParticleShape::Star,
    ...
    fn handle_mouse_interaction(
        ...
        if cursor_mode == 1 && self.settings.particle_count == 0 {
            // Create a small initial batch of particles for painting
            let initial_particle_count = 100;
            self.settings.particle_count = initial_particle_count;
    ...
    fn load_preset(&mut self, _preset_name: &str, _queue: &Arc<Queue>) -> crate::error::SimulationResult<()> {
        Ok(())
    }
    ...
    fn reset_runtime_state(
        ...
        // Reset particles
        let mut particles = Vec::with_capacity(self.settings.particle_count as usize);
        ...
        RNG.with(|rng| {
            let mut rng = rng.borrow_mut();
            for _ in 0..self.settings.particle_count {
                let x = rng.random_range(-1.0..1.0);
                let y = rng.random_range(-1.0..1.0);
    ...
    fn randomize_settings(
        ...
        self.settings.noise_scale = rng.random_range(0.05..0.3);
