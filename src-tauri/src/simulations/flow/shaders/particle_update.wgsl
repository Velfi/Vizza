...
fn deposit_trail(pos: vec2<f32>, particle_color: vec4<f32>) {
    ...
    let falloff = 1.0 - dist / f32(particle_radius);
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (particle_index >= sim_params.particle_limit) {
        return;
    }
    ...
    if (sim_params.cursor_active == 1u) {
        // Attract mode - spawn at cursor position with spray paint effect
        // Use cursor strength to control spawn rate (0.5 = 50% spawn rate, 1.0 = 100% spawn rate)
        let spawn_chance = fract(f32(particle_index) * 0.1234 + sim_params.time * 2.0);
        // Apply a scaling factor to reduce spawn rate - 1.0 cursor strength = 20% spawn rate
        let scaled_strength = sim_params.cursor_strength * 0.2;
        if (spawn_chance < scaled_strength) { // Use scaled cursor strength as spawn rate
            // Spray paint effect - circular distribution within radius
            let radius = f32(sim_params.cursor_size) * 0.01; // Convert cursor size to world radius
            
            // Generate random offset using multiple seeds for better distribution
            let seed1 = f32(particle_index) * 0.1234 + sim_params.time * 0.1;
            let seed2 = f32(particle_index) * 0.5678 + sim_params.time * 0.05;
            
            // Create circular spray pattern using polar coordinates
            let angle = fract(sin(seed1) * 43758.5453) * 2.0 * 3.14159; // Random angle 0 to 2π
            let distance = fract(cos(seed2) * 43758.5453); // Random distance 0 to 1
            
            // Apply circular spray effect - particles spread out in a circle
            ...
            should_reset = true; // Force reset to spawn at cursor
        } else if (should_reset) {
            // If particle is dying naturally, still spawn at cursor sometimes
            let natural_spawn_chance = fract(f32(particle_index) * 0.5678 + sim_params.time * 0.3);
            if (natural_spawn_chance < scaled_strength * 0.5) { // Use scaled cursor strength for natural spawn rate too
                // Circular spray paint effect for natural spawning too
        ...
        // Check if autospawn is enabled
        if (sim_params.particle_autospawn == 1u) {
            ...
            // Autospawn disabled - particle stays dead (age remains at lifetime)
            spawn_x = particle.position.x;
            spawn_y = particle.position.y;
