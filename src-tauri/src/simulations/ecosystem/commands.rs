...
pub async fn get_ecosystem_population_data(
    ...
    sim_manager.get_ecosystem_population(&gpu_ctx.device, &gpu_ctx.queue).await
}
...
pub async fn toggle_species_visibility(
    ...
    tracing::debug!("toggle_species_visibility called: role={}, variant={}", ecological_role, variant);
    ...
    if let Some(ecosystem_sim) = sim_manager.get_ecosystem_simulation_mut() {
        ecosystem_sim.toggle_species_visibility(ecological_role, variant, &gpu_ctx.queue)
